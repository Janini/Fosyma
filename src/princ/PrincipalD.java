...
import mas.agents.DummyMovingAgent;
...
public class PrincipalD {
	...
	public static void main(String[] args){
		...
		startAgents(agentList);
			...
			if(PLATFORM_isMAIN){
				...
				containerList.putAll(createAndConnectContainer("Pouet", PLATFORM_HOSTNAME, PLATFORM_ID, PLATFORM_PORT));
	...
	private static List<AgentController> createAgents(HashMap<String, ContainerController> containerList) {
		...
		c = containerList.get("Pouet");
		Assert.assertNotNull("This container does not exist",c);
		agentName="11PouetName";
		String gatekeeperName="GK";
		try {
			Object[] objtab=new Object[]{gatekeeperName};//used to give informations to the agent
			AgentController	ag=c.createNewAgent(agentName,ExploAgentBis.class.getName(),objtab);
			agentList.add(ag);
